...
SUBROUTINE DISPLACE_WRF_FIELDS_AD(th_compute, istep, qv_ind, moist, nmoist, moist_ad&
  ...
  REAL(fp), DIMENSION(kms:kme), INTENT(IN) :: znu, znw, c1h, c2h, c3h, c4h, c3f, c4f
...
SUBROUTINE CALC_PH_HYD_WRF_AD(qvapor, qvapor_ad, p, p_ad, pb, t, t_ad&
  ...
  REAL(fp), DIMENSION(kms:kme), INTENT(IN) :: c3h,c4h,c3f,c4f
    real(fp), INTENT(IN) :: ptop
  ...
  real(fp), dimension(ims:ime,jms:jme) :: pfu_ad,pfd_ad,phm_ad
...
!!$  REAL(fp) :: deta(kms:kme), znuw(kms:kme)
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: sum1_ad, sum2_ad, sum3_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme, 2) :: work2d
  REAL(fp), DIMENSION(ims:ime, jms:jme, 2) :: work2d_ad
  ...
  DO j= jms, jme
     DO i= ims, ime
        ...
        ELSE IF (t1 .GE. t_ref .AND. t1 .LT. const_47) THEN
           !liq phas poor es
           ew = es0*EXP(const_17*t1/(t1+243.5))
           CALL PUSHINTEGER4(1)
        ELSE
           tk = t(i, j)
     ...
     DO i= ime, ims, -1
        ...
        IF (branch .LT. 2) THEN
           ...
           ELSE
              t1 = t(i, j) - tf
              temp1 = t1/(t1+243.5)
              temp1_ad = es0*EXP(const_17*temp1)*const_17*ew_ad/(t1+243.5)
